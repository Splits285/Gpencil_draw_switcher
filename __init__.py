...
"warning": "Erasers as a keymap probably won't work well, also don\'t use X to clear text fields when changing Switch settings, blender lingers them unless you write exactly nothing, not even a space. More explanation on github (Documentation)",
...
#Key1-----------------Find current drawing keys according to users' specified Draw button & Value. The real trick behind this whole addon.-----------------------------------------------
                ...
                #This gets hit if the user lies to us and the keymap they specified doesn't exist
                if not keymap_entries:
                    self.report({'ERROR'}, "Error saving drawing keymaps. Likely the specified Draw keymap is nonexistent. No re-keymapping attempted.")
                    return {'FINISHED'}
...
#================================================================================
    ...
    def modal(self, context, event):
        ...
        if (event.type == TRIGGERKEY and event.value != TRIGGERSTATE) or event.type == 'ESC':

            #print(self.CDO_DrawKey, "drawkey...")
            #print(TRIGGERKEY, "opposite event.value achieved.")
            #print("\n")
